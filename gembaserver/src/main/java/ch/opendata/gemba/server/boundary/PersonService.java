/*
 * File: PersonService.java, created at 29.09.2012
 * $Id$
...
import ch.opendata.gemba.server.control.PersonAccess;
...
public class PersonService {
    ...
    public Person create(@PathParam("family") String aFamilyId,
            @PathParam("secToken") String aSecToken) {
        Family family = familyService.login(aFamilyId, aSecToken);
        if (family != null) {
            Person result = new Person();
            result.setFamilyFk(family);
            result.setPersonId(GembaFactory.createRandomUuid());
            personAccess.create(result);
            return result;
        }
        return null;
    }
    ...
    @Produces({"application/xml", "application/json"})
    @Consumes({"application/xml", "application/json"})
    public List<Person> get(@PathParam("family") String aFamilyId,
            @PathParam("secToken") String aSecToken) {
        Family family = familyService.login(aFamilyId, aSecToken);
        if (family != null) {
            return personAccess.findForFamily(family);
        }
        return null;
    }
    ...
    @Produces({"application/xml", "application/json"})
    @Consumes({"application/xml", "application/json"})
    public Person get(@PathParam("family") String aFamilyId, @PathParam("secToken") String aSecToken,
            @PathParam("person") String aPersonId) {
        Family family = familyService.login(aFamilyId, aSecToken);
        if (family != null) {
            return personAccess.findById(aPersonId);
        }
        return null;
    }
    ...
    @Produces({"application/xml", "application/json"})
    @Consumes({"application/xml", "application/json"})
    public Person update(@PathParam("family") String aFamilyId,
            @PathParam("secToken") String aSecToken, @PathParam("person") String aPersonId,
            Person aPerson) {
        Family family = familyService.login(aFamilyId, aSecToken);
        if (family != null) {
            Person result = personAccess.findById(aPersonId);
